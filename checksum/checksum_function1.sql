...
CREATE OR REPLACE
  ...
  TYPE line IS RECORD (length NUMBER);
...
BEGIN
  ...
  */
  lv_stmt := 'SELECT   LENGTH(text)'||CHR(10)
  ...
  /* Sum the lines into a total. */
  FOR i IN 1..lv_coll.COUNT LOOP
    lv_size := lv_size + lv_coll(i).length;
  END LOOP;
